...
function identifyPill() {
  ...
  if (imprint) desc.push(`imprint "${imprint}"`);
  resultEl.classList.remove("hidden");
  if (imprint) {
    // Attempt FDA lookup via imprint
      ...
      .then(data => {
        if (data.results?.length) {
          ...
          resultEl.innerHTML = `
            ...
            ${matches.join(", ")}<br><br>
            <em style="font-size:0.78rem;opacity:0.7">Always verify with a pharmacist. This is not a definitive identification.</em>
          `;
        } else {
...
function buildPillDescription(shape, color, imprint) {
  ...
  return `
    ...
    <a href="https://www.rxlist.com/pill-identification-tool/article.htm" target="_blank" style="color:var(--accent)">rxlist.com</a> 
    with these details.<br><br>
...
async function checkInteractions() {
  ...
  if (foundInteractions.length > 0) {
    ...
    resultEl.innerHTML = `<strong>✓ No major interactions found</strong> between the listed medicines in the FDA database.<br><br>
      <em style="font-size:0.78rem">This doesn't guarantee safety. Consult your pharmacist for a full review.</em>`;
  }
}
...
function addToCompare() {
  ...
  if (compareList.length >= 3) {
    showToast("Max 3 medicines to compare");
    return;
  }
